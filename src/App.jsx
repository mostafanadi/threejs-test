...
export default function App() {
  return (
    <Canvas style={{ width: "100dvw", height: "100dvh" }}  shadows dpr={[1, 1.5]} camera={{ position: [-1.5, 1, 5.5], fov: 45, near: 1, far: 200 }} eventSource={document.getElementById('root')} eventPrefix="client">
      {/* Lights */}
...
function CameraRig() {
  ...
  useFrame((state, delta) => {
    easing.damp2(state.camera.position, [-1 + (state.pointer.x * state.viewport.width) / 10, (1 + state.pointer.y) / 2,1], 0.5, delta)
